...
# Phase 17 (KernelCache): same link-filter helper as gpt_model.py.
# See that file for the algorithm rationale.
...
def group_query_attention(GQA_surrounding_path=None, GQA_kernel_path=None, inference=False):
...
def feed_forward_network(ffn_path=None, inference=False):
    if ffn_path is None:
...
def transformer_decoder_block(ffn_path=None, layernorm_path=None, residual_path=None, inference=False):
    ...
    mha = ReplicateGraph.apply(group_query_attention(inference=inference), "mha.%s")
    ...
    ffn = feed_forward_network(ffn_path, inference=inference)
...
def transformer_decoders(num_layers, decoder_template, inference=False):
    links = dict()
    decoders = list()
...
def llama(num_layers, embedding_path=None, regenerate=False, tpsp=False, inference=False):
    from . import CACHE_DIR
    import os

    # Phase 17: include mode in cache filename so modes don't share caches.
    mode_tag = "inference" if inference else "training"
    cache_filename = os.path.join(CACHE_DIR, f"llama_{num_layers}_{mode_tag}.csv")
    ...
    decoder_template = transformer_decoder_block(inference=inference)
    ...
    # Phase 17: skip the loss subgraph entirely in inference mode.
    if not inference:
